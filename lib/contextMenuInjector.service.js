var __extends = (this && this.__extends) || (function () {
    var extendStatics = Object.setPrototypeOf ||
        ({ __proto__: [] } instanceof Array && function (d, b) { d.__proto__ = b; }) ||
        function (d, b) { for (var p in b) if (b.hasOwnProperty(p)) d[p] = b[p]; };
    return function (d, b) {
        extendStatics(d, b);
        function __() { this.constructor = d; }
        d.prototype = b === null ? Object.create(b) : (__.prototype = b.prototype, new __());
    };
})();
import { ContextMenuContentComponent } from './contextMenuContent.component';
import { Injectable } from '@angular/core';
import { InjectionService } from './injection/injection.service';
import { InjectionRegistry } from './injection/injection-registry.service';
var ContextMenuInjectorService = (function (_super) {
    __extends(ContextMenuInjectorService, _super);
    function ContextMenuInjectorService(injectionService) {
        var _this = _super.call(this, injectionService) || this;
        _this.injectionService = injectionService;
        _this.type = ContextMenuContentComponent;
        return _this;
    }
    return ContextMenuInjectorService;
}(InjectionRegistry));
export { ContextMenuInjectorService };
ContextMenuInjectorService.decorators = [
    { type: Injectable },
];
/** @nocollapse */
ContextMenuInjectorService.ctorParameters = function () { return [
    { type: InjectionService, },
]; };
//# sourceMappingURL=contextMenuInjector.service.js.map